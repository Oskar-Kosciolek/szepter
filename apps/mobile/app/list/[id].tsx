...
export default function ListDetailScreen() {
  ...
  const { items, loading, text, setText, deadline, setDeadline, saving, handleAdd, handleToggle, handleDelete } = useListDetail(id)

  return (
    <SafeAreaView style={s.container}>
      <View style={s.header}>
        <Pressable onPress={() => router.back()} hitSlop={12}><ArrowLeft size={22} color="#fff" /></Pressable>
        <Text style={s.title} numberOfLines={1}>{decodeURIComponent(title)}</Text>
      </View>
      <View style={s.inputWrap}>
        <View style={s.inputRow}>
          <TextInput style={s.input} placeholder="Nowy element..." placeholderTextColor="#444" value={text} onChangeText={setText} />
      ...
      {loading
        ...
        : <FlatList
            ...
            renderItem={({ item }) => (
              <ListItemRow item={item} onToggle={() => handleToggle(item)} onDelete={() => handleDelete(item)} />
            )}
